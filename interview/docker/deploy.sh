...
base() {
  ...
  if ! $compose_cmd \
    ...
    interview-eureka; then
    echo "❌ 基础设施启动失败"
    get_service_logs interview-eureka
    exit 1
  fi
  ...
  # 验证Eureka是否真正启动
  if ! docker-compose exec -T interview-eureka \
    curl -sfS http://interview-eureka:8761/actuator/health | grep -q 'UP'; then
    echo "❌ Eureka未正常启动"
    get_service_logs interview-eureka
    exit 1
  fi
...
build() {
  ...
  # 强制清理旧镜像（关键修改）
  echo "🧹 清理旧镜像..."
  docker-compose rm -fvs interview-eureka interview-auth interview-gateway interview-product 2>/dev/null
  docker rmi $(docker images | grep 'interview-' | awk '{print $3}') 2>/dev/null || true
...
modules() {
  ...
  # 确保Eureka已运行
  if ! docker-compose ps | grep -q "interview-eureka"; then
    base
  fi
  ...
  # 使用wget进行健康检查（已验证可用）
  while ! docker-compose exec -T interview-eureka \
    wget -qO- http://interview-eureka:8761/actuator/health | grep -q '"status":"UP"'; do
    ((retry_count++))

    # 每3次重试显示最新日志
    if (( retry_count % 3 == 0 )); then
      echo "📜 最近日志："
      docker-compose logs --tail=3 interview-eureka | awk '{print "    | " $0}'
    fi
  ...
  echo "✅ Eureka已就绪，状态："
  docker-compose exec -T interview-eureka wget -qO- http://interview-eureka:8761/actuator/health | jq . 2>/dev/null || \
    docker-compose exec -T interview-eureka wget -qO- http://interview-eureka:8761/actuator/health
...
clean() {
  ...
  docker-compose ps -a | grep -E 'interview-mysql|interview-ldap'
}
...
purge() {
  ...
  # 清理未被使用的业务镜像（按名称匹配）
  echo "▸ 清理业务镜像..."
  for img in $(docker images --format "{{.Repository}}:{{.Tag}}" | grep 'interview-'); do
    # 检查是否有容器在使用该镜像
...
nginx() {
  ...
  local image_name=interview-nginx
  ...
  echo "正在停止 nginx 操作......."
  docker-compose stop interview-nginx

  # 清理旧资源（仅针对 nginx）
  echo "▸ 清理旧版 nginx 资源..."
  # 1. 停止并移除当前运行的 nginx 容器
  docker-compose rm -fsv interview-nginx 2>/dev/null || true

  # 2. 删除所有 nginx 相关镜像（按名称过滤）
  docker images --format "{{.Repository}}:{{.Tag}}" | grep "^interview-nginx" | xargs -r docker rmi 2>/dev/null || true
  # 3. 清理 dangling 镜像（可选）
